...
def detect_model_type(*checkpoint_paths: str | None) -> str:
    """Return dramabox, distilled, or dev from paths/metadata.

    DramaBox ships as a LoRA-merged LTX-2.3 audio model and uses the guided
    server path. Distilled LTX swaps need a different default sigma schedule
    and no guidance by default.
    """
    env_type = os.environ.get("DRAMABOX_MODEL_TYPE", "").strip().lower()
    if env_type and env_type != "auto":
        return env_type

    joined = " ".join(str(p or "") for p in checkpoint_paths).lower()
    if "dramabox" in joined:
        return "dramabox"
    if "distilled" in joined:
        return "distilled"
    if "dev" in joined:
        return "dev"
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        try:
            ...
            version = str(metadata.get("model_version", "")).lower()
            if "distilled" in version:
                return "distilled"
            if "dev" in version:
                return "dev"
        except Exception:
...
def model_defaults(model_type: str) -> dict:
    model_type = (model_type or "dramabox").lower()
...
class TTSServer:
    def __init__(self, checkpoint=None, full_checkpoint=None, gemma_root=None,
        ...
        requested_type = (model_type or "").lower()
        if not requested_type or requested_type == "auto":
            requested_type = detect_model_type(self.checkpoint, self.full_checkpoint)
        self.model_type = requested_type
        self.default_params = model_defaults(self.model_type)
        ...
        logging.info(f"TTSServer loading {self.model_type} model on {device}...")
        t0 = time.time()
        self._load_all()
    ...
    def generate(self, prompt, voice_ref=None, cfg_scale=None, stg_scale=None,
                 duration_multiplier=1.1, seed=42, ref_duration=10.0,
                 rescale_scale=None, gen_duration: float = 0.0,
                 modality_scale=None, steps=None, fps=None):
        ...
        """
        t_total = time.time()
        cfg_scale = self.default_params["cfg_scale"] if cfg_scale is None else float(cfg_scale)
        stg_scale = self.default_params["stg_scale"] if stg_scale is None else float(stg_scale)
        rescale_scale = self.default_params["rescale_scale"] if rescale_scale is None else rescale_scale
        modality_scale = (
            self.default_params["modality_scale"] if modality_scale is None else float(modality_scale)
        )
        fps = self.default_params["fps"] if fps is None else float(fps)
        steps = self.default_params["steps"] if steps is None else int(steps)
        ...
        # Prompt encode
        t0 = time.time()
        needs_negative = cfg_scale > 1.0
        prompts = [prompt, DEFAULT_NEG] if needs_negative else [prompt]
        ctx = self._prompt_encoder(prompts, streaming_prefetch_count=None)
        a_ctx = ctx[0].audio_encoding
        a_ctx_neg = ctx[1].audio_encoding if needs_negative else None
        ...
        needs_guidance = cfg_scale > 1.0 or stg_scale > 0.0 or modality_scale > 1.0
        if needs_guidance:
            guider = MultiModalGuider(
                params=MultiModalGuiderParams(
                    cfg_scale=cfg_scale, stg_scale=stg_scale,
                    stg_blocks=[29], rescale_scale=resc, modality_scale=modality_scale,
                ),
            ...
            logging.info(
                "GuidedDenoiser: cfg=%.2f stg=%.2f rescale=%s modality=%.2f",
                cfg_scale, stg_scale, resc, modality_scale,
            )
        else:
        ...
        # Sigmas
        if self.model_type == "distilled" and (not steps or steps <= 0):
            sigmas = torch.tensor(DISTILLED_SIGMA_VALUES, dtype=torch.float32, device=self.device)
            step_label = f"{len(DISTILLED_SIGMA_VALUES) - 1} distilled"
        else:
            step_count = steps if steps and steps > 0 else 30
            sigmas = LTX2Scheduler().execute(steps=step_count, latent=state.latent).to(self.device)
            step_label = str(step_count)
        ...
        logging.info(f"Denoise ({step_label} steps): {time.time()-t0:.2f}s")
